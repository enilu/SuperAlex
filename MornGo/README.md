...
一个为儿童晨间流程打造的轻量级激励游戏。通过任务选择、倒计时挑战、音效反馈与成就体系，帮助孩子养成良好的时间管理习惯，同时降低家长的催促压力。
...
- 主要功能：任务选择模式、倒计时挑战、任务完成反馈、成就系统、数据统计、任务自定义配置、设置分页面

## 游戏模式

### 🎯 任务选择模式（v1.0.0新增）

**适用场景**：工作日和周末起居时间不固定，无法按预设顺序完成任务。

**玩法说明**：
1. **任务选择界面**：游戏开始后，显示所有可完成的任务列表
2. **自由选择任务**：点击想要完成的任务，开始倒计时挑战
3. **倒计时挑战**：每个任务设定完成时限（如：起床2分钟、穿衣3分钟）
4. **闯关判定**：
   - ✅ **成功**：在倒计时结束前点击"我完成了！"，显示"闯关成功！"
   - ❌ **失败**：倒计时结束，播放提示音，显示"时间到了！下次加油！"
5. **继续挑战**：完成后返回任务列表，选择下一个任务
6. **完美通关**：完成所有任务后，显示庆祝界面和勋章
...
```javascript
{
    ...
    name: "起床",
    icon: "🛏️",
    countdownSeconds: 120  // 倒计时2分钟（120秒）
}
```
...
- 当前版本：`v1.0.0`
- 重要更新：
  - **新增任务选择模式**：不按固定顺序，自由选择要完成的任务
  - **倒计时挑战系统**：每个任务独立倒计时，时间到播放提示音
  - **闯关成功/失败判定**：倒计时结束前完成算成功，否则失败
  - **任务配置优化**：支持设置每个任务的倒计时秒数
  - 新增设置独立页面与分页面路由
  - 引入统一 SVG 图标体系与注入工具
  - 优化任务配置布局与底部按钮的响应式样式
